var input = "1713\n" +
    ...
    "1569\n" +
    "1970";
var output = "289270976";

function find_sum_2020_part2(lines) {
        ...
        numbers.forEach(function (w) { //  browse using index
            if (j > i) {//  case the second number is less than the first number
                if (v + w < 2020) {//  check the sum
                    var k = 0;
                    numbers.forEach(function (x) { //  browse using index
                        if (k > j) {//  case the third number is less than the second number
                            if (v + w + x === 2020) {//  check the sum
                                res = v * w * x; //  return the product
                            }
                        }
                        k += 1;
                    });
...
describe('Day 1', function() {
    it('SumTwoNumberspart22020', function() {
        var lines = input.split("\n"); // get lines
        var res = find_sum_2020_part2(lines); // process
        //console.log(res);

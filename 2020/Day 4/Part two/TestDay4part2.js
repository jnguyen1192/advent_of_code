var input = "hcl:5d90f0 cid:270 ecl:#66dc9c hgt:62cm byr:1945 pid:63201172 eyr:2026\n" +
    "\n" +
...
function get_nb_valid_password(lines) {
    function isNumeric(value) {
        return /^-?\d+$/.test(value);
    }

    function is_digit_interval(min, max, nb_digit, key, password_to_check) {
        return min <= parseInt(password_to_check[key]) &&  parseInt(password_to_check[key]) <= max && password_to_check[key].length === nb_digit && isNumeric(password_to_check[key]); // check rule with interval min max and length of digits
    }

    function is_hgt_interval(min, max, unit, key, password_to_check) {
        if (password_to_check[key].slice(password_to_check[key].length - 2) === unit) {
            return min <= parseInt(password_to_check[key].slice(0, password_to_check[key].length - 2)) && parseInt(password_to_check[key].slice(0, password_to_check[key].length - 2)) <= max; // check interval of height
        } // check unit of height
        return false;
    }

    function is_valid_string(length, valid_chars, password) {
        if(password.length === length) { // check length of chars
            for(var i = 0; i < password.length; i++) {
                if (valid_chars.indexOf(password[i]) === -1) { // check if each char are in valid chars
                    return false;
                }
            }
            return true;
    ...
    function is_valid_integrity(key, password_to_check) {
        if (key === "byr") {
        ...
        if (key === "hgt") {
            if (password_to_check[key].slice(password_to_check[key].length - 2) === "cm") {
                return is_hgt_interval(150, 193, "cm", key, password_to_check);  // If cm, the number must be at least 150 and at most 193
            } else if (password_to_check[key].slice(password_to_check[key].length - 2) === "in") {
        ...
        if(key === "hcl") {
            if (password_to_check[key][0] === "#") {
                return is_valid_string(6, "0123456789abcdef", password_to_check[key].slice(1));  // hcl (Hair Color) - a # followed by exactly six characters 0-9 or a-f
            }
            return false;
        }
        if(key === "ecl") {
            return ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].indexOf(password_to_check[key]) >= 0; // ecl (Eye Color) - exactly one of: amb blu brn gry grn hzl oth
        }
        if (key === "pid") {
    ...
    function is_valid_password(fields_valid, password_to_check) {
        var nb_key_in_password = 0;
        for (var key in password_to_check){ // for each keys
            if(!is_valid_integrity(key, password_to_check)) { // check rules before count
                return false;
            }
            if (fields_valid.indexOf(key) >= 0) {  // is key valid
                nb_key_in_password += 1; // increment nb key
            }
        }
        return nb_key_in_password === fields_valid.length; // test if nb key is correct
    }


    var fields_valid = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"];  // all key present and valid

    var password_to_check = {}; // initialize a password
    var passwords_to_check = [];  // initialize password list
    var key_value = [];

    lines.forEach(function(line) { // for each line
        line.split(" ").forEach(function(code) { // get the keys
            if(line === "") {  // case it is a blank line
                passwords_to_check.push(password_to_check);  // add a new password on list
                password_to_check = {};  // clear the password
                return;
            }
            key_value = code.split(":");  // extract key and value
            password_to_check[key_value[0]] = key_value[1];  // add a new value on dict with its key
        });
    });
    passwords_to_check.push(password_to_check);  // add the last new password on list

    var nb_valid_password = 0;  // initialize nb valid password
    var nb_password = 0
    passwords_to_check.forEach(function(password_to_check) { // for each passwords
...
describe('Day 4', function() {
    it('NbValidPasswordPart22020', function() {
        var lines = input.split("\n"); // get lines
        var res = get_nb_valid_password(lines); // process
        //console.log(res);

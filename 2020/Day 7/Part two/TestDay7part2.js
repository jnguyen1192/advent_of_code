...
function get_sum_group_count_yes_all(lines){
    var letters = "abcdefghijklmnopqrstuvwxyz";  // initialize all char to count
    var group_yes = [];  // initialize group of yes
    var group_count = {};  // initialize each group letter count
    group_count["nb_group"] = 0
    var yes_all_count;
    for(var i = 0; i < letters.length; i++){  // for each letter
        group_count[letters[i]] = 0;  // initialize count as 0
    }

    function yes_all(group_count){
        var count = 0
        for (var key in group_count) {  // for each answer
            if(group_count["nb_group"] === group_count[key] && key !== "nb_group") {  // if the number of yes equals number of people
                count += 1;  // return 1
            }
        }
        return count;
    ...
    lines.forEach(function(line) {  // for each people
        for(var i = 0; i < line.length; i++){ // for each yes answer
            group_count[line[i]] += 1;  // set letter count to one
        }
        if(line === "") {
            group_yes.push(yes_all(group_count));  // sum the values of yes and add it to the final group to count yes
            for(var i = 0; i < letters.length; i++){  // for each letter
                group_count[letters[i]] = 0;  // initialize count as 0
            }
    ...
    group_yes.push(yes_all(group_count));  // get the sum of all yes for each group
    return group_yes.reduce(function(a, b) { return a + b; }, 0)  // return nb of valid password
}


var assert = require('assert'); // Unit test library
describe('Day 6', function() {
    it('GroupYesPart2', function() {
        var lines = input.split("\n"); // get lines
        var res = get_sum_group_count_yes_all(lines); // process
        console.log(res);

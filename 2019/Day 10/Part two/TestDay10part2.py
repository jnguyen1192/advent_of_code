...
class TestDay10part1(unittest.TestCase):
    ...
    def test_day_10_part_1(self):
        ...
        def get_next_asteroid_using_map_and_laser_position(tmp_map, station, laser_position=False):
            ...
            # case right-top
            print("Case right-top")
            print(y_laser)
            print(x_laser)
            if y_laser < 0 and abs(x_laser) >= abs(y_laser) and x_laser > 0:
                print("Right top")
                for x in range(x_laser, width - x_station):
                    for y in range(-y_station, 0, 1):
                        # and not on diag
                        print("range",x_laser, width - x_station)
                        print((x + x_station, y + y_station))
                        print((x, y))
                        if tmp_map[y + y_station][x + x_station] == "#" and abs(x) != abs(y) and abs(x) > abs(y):
                            return (x + x_station, y + y_station)
            # case diag bottom-right
            for i in range(y_station + 1,  height):
                #print(i + y_station, i + x_station)
                if tmp_map[y_station + i][i + x_station] == "#":
                    #if (i + x_station, y_station + i) == (8, 3):
                    #    print("WHY")
            ...
            for i in range(y_station + 1,  height):
                #print(i + y_station, i + x_station)
                if tmp_map[y_station + i][x_station - i] == "#":
                    #if (i + x_station, y_station + i) == (8, 3):
                    #    print("WHY")
                    return (x_station - i, y_station + i)
            # case diag top-left
            for i in range(1, y_station - 1):
                #print(i + y_station, i + x_station)
                if tmp_map[y_station - i][x_station - i] == "#":
